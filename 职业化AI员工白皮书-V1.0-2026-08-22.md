...
本白皮书定义职业化AI员工品类，并提供首个完整工程实现：ERDL 规范、形式化验证工具、301 条跨实现审计验证向量、运行时框架均已开源，治理基础设施内测中。企业今日可申请内测，独立开发者今日即可验证。
...
「中立性是被测出来的」落地为 301 条可独立重算的验证向量，分属两大层——决策证据层与表达式内核层。两层的验证范式不同：决策证据层是密码学验证（防篡改，重算 JCS + SHA-256 哈希），表达式内核层是语义验证（正确性，按规范求值并比对结果）。
...
**表达式内核层 · 223 条**（`v-engine-vectors.json`）——覆盖表达式树的确定性求值：
...
**表达式内核层的独立重算要求**：独立实现须按 ERDL SPEC v2.0 解析表达式树并执行求值，结果与预期值、类型逐项一致——这不是对某个实现的单元测试，而是对公开规范的语义对拍。
...
**版本谱系说明（请仔细阅读）**：上述独立验证完成于 v1.3 审计向量（AV-001–AV-013），证明的是方法论的可行性；§7.1 现行的 78 条 v1.5 向量**尚未被独立重算**——参考实现已通过自验，独立验证进行中，正等待第 2、第 3 个 Runner。v1.5 体系承袭同一套 JCS + SHA-256 机制与金丝雀设计，下文的 Runner 征集令正是为此而发。我们不用旧版本的验证结果为新版本背书：新版本必须重新被测。

这意味着：一个从未看过我们任何代码的人，仅凭公开规范，独立重算出了与我们逐字节一致的哈希——**中立性不是宣称的，是被独立测出来的。**

**Runner 征集令**：我们现在公开征集第 2、第 3 个独立实现者（Runner）。验证者将获得：
...
为此，我们建立了公开的可验证基准：**301 条验证向量，分属两种验证范式**——决策证据层 78 条（V-DO-v15，密码学验证）与表达式内核层 223 条（V-ENGINE/V-GLOSS/V-PROJ，语义验证），两层均已全量发布，签名层向量将在此后逐步发布——覆盖行为边界表达、决策证据防篡改、合规字段、业务场景与多方审计视角。任何第三方均可按公开规范独立重算——**能骗过人，骗不过数学。**
...
`verify:vengine` 对 223 条表达式内核向量按 ERDL SPEC v2.0 逐一求值并与预期值比对——第 3 步验证的是密码学层（防篡改），这一步验证的是语义层（正确性），两步都跑完，才是完整验证。
