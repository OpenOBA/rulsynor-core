...
// Canonical evaluation context: Entity namespaces at the top level (`tool.name`/`tool.args.*`).
...
if (jsonMode) {
  ...
  console.log(
    '🪪 Employee ID:' + ' ' + (aid ?? '(not selected: CN GB/Z 185 AID requires CN jurisdiction)'),
  );
  console.log(
    '📊 Jurisdiction:' +
      ' ' +
