...
export class RuleQualityGate {
  ...
  check(rules: RuleDefinition[]): QualityGateReport {
    ...
    if (errCount > 0) {
      ...
      logQg.warn(`⚠️ ${warnCount} warning(s)${infoCount > 0 ? `, ${infoCount} info` : ''} in ${total} rules`)
    } else if (infoCount > 0) {
      logQg.log(`ℹ️ ${infoCount} naming suggestion(s) in ${total} rules (SHOULD level — non-blocking)`)
    } else if (total > 0) {
