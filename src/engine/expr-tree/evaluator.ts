...
export class ExprTreeEvaluator {
    ...
    switch (node.type) {
      ...
      case 'in': {
        ...
        if (r.value.length > 256) {
          warnings.push({ kind: 'array_over_limit', message: 'in 列表超过 256 项上限', nodeType: 'in' })
          return ok(false, warnings)
        }
      ...
      case 'length': {
        ...
        if (typeof v === 'string') {
          ...
          r.warnings.push({ kind: 'type_mismatch', message: 'length 仅支持字符串/数组', nodeType: 'length' })
          out = 0
        }
      ...
      case 'quantifier': {
        ...
        if (!Array.isArray(over.value)) {
          const w: EvalWarning = { kind: 'type_mismatch', message: '量词 over 必须为数组', nodeType: 'quantifier' }
          return ok(false, [...over.warnings, w])
        }
        const arr = over.value as unknown[]
        if (arr.length === 0) {
          const w: EvalWarning = { kind: 'quantifier_empty', message: `量词 ${node.kind} 空数组折叠为 false`, nodeType: 'quantifier' }
          return ok(false, [...over.warnings, w])
        }
      ...
      case 'aggregate': {
        ...
        if (over.errored) return over
        if (!Array.isArray(over.value)) {
          const w: EvalWarning = { kind: 'type_mismatch', message: 'aggregate over 必须为数组', nodeType: 'aggregate' }
  ...
  private numCompare(left: unknown, right: unknown, op: string, warnings: EvalWarning[]): boolean {
    ...
    const rr = this.toRational(right)
    if (lr === null || rr === null) {
      warnings.push({ kind: 'type_mismatch', message: `不支持 ${typeof left} 与 ${typeof right} 的大小比较`, nodeType: 'compare' })
      return false
    }
  ...
  private stringMatch(op: string, left: unknown, right: unknown, warnings: EvalWarning[]): boolean {
    if (typeof right !== 'string') {
      warnings.push({ kind: 'type_mismatch', message: '字符串运算右操作数必须为字符串', nodeType: 'string' })
      return false
    }
    ...
    switch (op) {
      case 'contains': {
        ...
        if (typeof left !== 'string') {
          warnings.push({ kind: 'type_mismatch', message: 'contains 左操作数必须为字符串', nodeType: 'string' })
          return false
        }
      ...
      case 'match': {
        try {
          ...
          if (ln.length > REGEX_MAX_INPUT_LENGTH) {
            warnings.push({ kind: 'regex_re_dos', message: `正则匹配输入超长（${ln.length} > ${REGEX_MAX_INPUT_LENGTH}），已截断匹配`, nodeType: 'string' })
          }
  ...
  private arith(op: string, values: unknown[], warnings: EvalWarning[]): EvalResult {
    ...
    for (const v of values) {
      ...
      if (r === null) {
        warnings.push({ kind: 'type_mismatch', message: `算术操作数非数值：${typeof v}`, nodeType: 'arith' })
    ...
    switch (op) {
      ...
      case 'sub': {
        // 减法不可结合 → 必须二元
        if (rats.length !== 2) {
          warnings.push({ kind: 'type_mismatch', message: `sub 必须两个操作数，实际 ${rats.length}`, nodeType: 'arith' })
          return ok(null, warnings)
        }
      ...
      case 'div': {
        // 除法不可结合 → 必须二元
        if (rats.length !== 2) {
          warnings.push({ kind: 'type_mismatch', message: `div 必须两个操作数，实际 ${rats.length}`, nodeType: 'arith' })
          return ok(null, warnings)
        }
        if (rats[1].num === 0n) {
      ...
      case 'round': {
        if (rats.length !== 1) {
          warnings.push({ kind: 'type_mismatch', message: `round 必须一个操作数，实际 ${rats.length}`, nodeType: 'arith' })
          return ok(null, warnings)
        }
  ...
  private dateAdd(unit: string, base: unknown, amount: unknown, warnings: EvalWarning[]): EvalResult {
    ...
    if (d === null) {
      warnings.push({ kind: 'invalid_date', message: `date_add 基准日期非法：${String(base)}`, nodeType: 'date_add' })
      return ok(null, warnings)
    }
    const n = this.toRational(amount)
    if (n === null) {
      warnings.push({ kind: 'type_mismatch', message: `date_add 步长必须为数值：${typeof amount}`, nodeType: 'date_add' })
      return ok(null, warnings)
    }
    ...
    switch (unit) {
      ...
      default:
        warnings.push({ kind: 'type_mismatch', message: `未知 date_add 单位：${unit}`, nodeType: 'date_add' })
        return ok(null, warnings)
    }
  }
  ...
  private datePart(unit: string, value: unknown, warnings: EvalWarning[]): EvalResult {
    ...
    if (d === null) {
      warnings.push({ kind: 'invalid_date', message: `date_part 日期非法：${String(value)}`, nodeType: 'date_part' })
      return ok(null, warnings)
    }
    switch (unit) {
      case 'year': return ok(getYear(d), warnings)
      case 'month': return ok(getMonth(d) + 1, warnings) // 对齐人类习惯：1=一月
      ...
      default:
        warnings.push({ kind: 'type_mismatch', message: `未知 date_part 分量：${unit}`, nodeType: 'date_part' })
        return ok(null, warnings)
    }
  }
  ...
  private monthLastDay(value: unknown, warnings: EvalWarning[]): EvalResult {
    const d = this.toDate(value)
    if (d === null) {
      warnings.push({ kind: 'invalid_date', message: `month_last_day 日期非法：${String(value)}`, nodeType: 'month_last_day' })
      return ok(null, warnings)
    }
  ...
  private aggregate(fn: string, arr: unknown[], warnings: EvalWarning[]): EvalResult {
    switch (fn) {
      ...
      case 'avg': {
        if (arr.length === 0) {
          warnings.push({ kind: 'aggregate_empty', message: 'avg 数组为空，安全折叠为 false', nodeType: 'aggregate' })
          return ok(false, warnings)
        }
      ...
      case 'min': {
        ...
        if (rats === null) return ok(null, warnings)
        if (rats.length === 0) {
          warnings.push({ kind: 'aggregate_empty', message: 'min 数组为空，安全折叠为 false', nodeType: 'aggregate' })
          return ok(false, warnings)
        }
        let m = rats[0]
      ...
      case 'max': {
        ...
        if (rats === null) return ok(null, warnings)
        if (rats.length === 0) {
          warnings.push({ kind: 'aggregate_empty', message: 'max 数组为空，安全折叠为 false', nodeType: 'aggregate' })
          return ok(false, warnings)
        }
        let m = rats[0]
        for (let i = 1; i < rats.length; i++) if (rationalCompare(rats[i], m) > 0) m = rats[i]
      ...
      default: return err(`未知聚合函数 ${fn}`, warnings)
    }
  }
  ...
  private toRationalArray(arr: unknown[], fn: string, warnings: EvalWarning[]): Rational[] | null {
    ...
    for (const v of arr) {
      const r = this.toRational(v)
      if (r === null) {
        warnings.push({ kind: 'type_mismatch', message: `${fn} 数组含非数值元素：${typeof v}`, nodeType: 'aggregate' })
        return null
      }
      rats.push(r)
    }
