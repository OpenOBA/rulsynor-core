...
export function fromSExpr(input: unknown): ExprNode {
  ...
  if (keys.length !== 1) {
    throw new SExprParseError(`每个 S-expression 节点必须恰有一个键，实际 ${keys.length} 个：${keys.join(',')}`)
  }
  const key = keys[0]
  const val = obj[key]
  ...
  switch (key) {
    ...
    case 'between': {
      if (!Array.isArray(val) || val.length !== 3) throw new SExprParseError('between 必须三个操作数')
      return { type: 'between', value: fromSExpr(val[0]), min: fromSExpr(val[1]), max: fromSExpr(val[2]) }
    }
    case 'days_between': {
      if (!Array.isArray(val) || val.length !== 2) throw new SExprParseError('days_between 必须两个操作数')
      return { type: 'days_between', from: fromSExpr(val[0]), to: fromSExpr(val[1]) }
    }
    case 'epoch_ms':
      return { type: 'epoch_ms', arg: fromSExpr(val) }
    case 'date_add': {
      if (typeof val !== 'object' || val === null) throw new SExprParseError('date_add 的值必须是对象 {unit,base,amount}')
      const o = val as Record<string, unknown>
      if (!DATE_ADD_UNITS.includes(o.unit as DateAddUnit)) throw new SExprParseError(`未知 date_add 单位：${o.unit}`)
      return { type: 'date_add', unit: o.unit as DateAddUnit, base: fromSExpr(o.base), amount: fromSExpr(o.amount) }
    }
    case 'date_part': {
      if (typeof val !== 'object' || val === null) throw new SExprParseError('date_part 的值必须是对象 {unit,arg}')
      const o = val as Record<string, unknown>
      if (!DATE_PART_UNITS.includes(o.unit as DatePartUnit)) throw new SExprParseError(`未知 date_part 分量：${o.unit}`)
  ...
  if (key === 'not_between') {
    if (!Array.isArray(val) || val.length !== 3) throw new SExprParseError('not_between 必须三个操作数')
  ...
  if (QUANT_KINDS.includes(key as QuantifierKind)) {
    if (typeof val !== 'object' || val === null) throw new SExprParseError(`${key} 的值必须是对象 {binding,over,predicate}`)
