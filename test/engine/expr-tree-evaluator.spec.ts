...
import { toDecimalString, fromDecimalString, add, div } from '../../src/engine/expr-tree/fixed-point.js'
...
describe('ExprTreeEvaluator — 逻辑节点', () => {
  ...
  it('or 任一真 → true', () => {
    const node: ExprNode = { type: 'or', args: [
      { type: 'literal', value: false },
      { type: 'literal', value: true },
    ] }
    expect(ev.evaluate(node, ctx({})).value).toBe(true)
...
describe('ExprTreeEvaluator — 比较节点（严格类型匹配 §11.2）', () => {
  it('eq number', () => {
    const node: ExprNode = { type: 'compare', op: 'eq', left: { type: 'field', field: 'age' }, right: { type: 'literal', value: 35 } }
  ...
  it('字符串 "100" gt 50 → false（无隐式转换）', () => {
    const node: ExprNode = { type: 'compare', op: 'gt', left: { type: 'field', field: 'amount' }, right: { type: 'literal', value: 50 } }
...
describe('ExprTreeEvaluator — 集合/字符串/存在/长度/区间', () => {
  it('in', () => {
    const node: ExprNode = { type: 'in', left: { type: 'field', field: 'cat' }, right: { type: 'literal', value: ['罕见重疾', '普通重疾'] } }
    expect(ev.evaluate(node, ctx({ cat: '罕见重疾' })).value).toBe(true)
  })
  it('contains', () => {
    const node: ExprNode = { type: 'string', op: 'contains', left: { type: 'field', field: 'cmd' }, right: { type: 'literal', value: 'rm' } }
  ...
  it('between 闭区间', () => {
    const node: ExprNode = { type: 'between', value: { type: 'field', field: 'age' }, min: { type: 'literal', value: 16 }, max: { type: 'literal', value: 60 } }
...
describe('ExprTreeEvaluator — 量词（E8 空数组安全折叠）', () => {
  it('all 全部满足 → true', () => {
    const node: ExprNode = { type: 'quantifier', kind: 'all', binding: 'x', over: { type: 'field', field: 'items' }, predicate: { type: 'compare', op: 'gt', left: { type: 'var', path: 'x' }, right: { type: 'literal', value: 0 } } }
    expect(ev.evaluate(node, ctx({ items: [1, 2, 3] })).value).toBe(true)
  })
  it('any 部分满足 → true', () => {
    const node: ExprNode = { type: 'quantifier', kind: 'any', binding: 'x', over: { type: 'field', field: 'items' }, predicate: { type: 'compare', op: 'gt', left: { type: 'var', path: 'x' }, right: { type: 'literal', value: 2 } } }
    expect(ev.evaluate(node, ctx({ items: [1, 2, 3] })).value).toBe(true)
  })
  ...
  it('none 空数组 → false（安全折叠）', () => {
    const node: ExprNode = { type: 'quantifier', kind: 'none', binding: 'x', over: { type: 'field', field: 'items' }, predicate: { type: 'literal', value: true } }
...
describe('ExprTreeEvaluator — 算术 / 时间 / 聚合', () => {
  it('arith mul 返回有理数（精确，中间不舍入）', () => {
    const node: ExprNode = { type: 'arith', op: 'mul', args: [{ type: 'field', field: 'share' }, { type: 'field', field: 'price' }] }
  ...
  it('arith div 三分之四 → 精确有理数 num=4 den=3', () => {
    const node: ExprNode = { type: 'arith', op: 'div', args: [{ type: 'literal', value: 4 }, { type: 'literal', value: 3 }] }
  ...
  it('arith div 除零 → 警告 + null（E12 由调用方折叠）', () => {
    const node: ExprNode = { type: 'arith', op: 'div', args: [{ type: 'literal', value: 10 }, { type: 'literal', value: 0 }] }
    const r = ev.evaluate(node, ctx({}))
    expect(r.warnings.some((w) => w.kind === 'division_by_zero')).toBe(true)
  })
  it('arith div 三个操作数 → 类型不匹配警告（不可结合，强制二元）', () => {
    const node: ExprNode = { type: 'arith', op: 'div', args: [{ type: 'literal', value: 10 }, { type: 'literal', value: 2 }, { type: 'literal', value: 5 }] }
    const r = ev.evaluate(node, ctx({}))
    expect(r.warnings.some((w) => w.kind === 'type_mismatch')).toBe(true)
  })
  ...
  it('compare 使用有理数精确比较（0.1+0.2 不等于 0.3 的浮点陷阱）', () => {
    ...
    const sum: ExprNode = { type: 'arith', op: 'add', args: [{ type: 'literal', value: 0.1 }, { type: 'literal', value: 0.2 }] }
    const node: ExprNode = { type: 'compare', op: 'eq', left: sum, right: { type: 'literal', value: 0.3 } }
    const r = ev.evaluate(node, ctx({}))
    expect(r.value).toBe(true)
  })
  it('days_between', () => {
    const node: ExprNode = { type: 'days_between', from: { type: 'literal', value: '2026-01-01' }, to: { type: 'literal', value: '2026-01-11' } }
    expect(ev.evaluate(node, ctx({})).value).toBe(10)
  })
  // ══ 时间节点族（Phase A：date_add / date_part / month_last_day）══
  it('date_add years 顺加（对年）', () => {
    const node: ExprNode = { type: 'date_add', unit: 'years', base: { type: 'literal', value: '2024-01-15' }, amount: { type: 'literal', value: 2 } }
  ...
  it('date_add months 月末回退（§202：1月31 + 1月 → 2月28）', () => {
    const node: ExprNode = { type: 'date_add', unit: 'months', base: { type: 'literal', value: '2024-01-31' }, amount: { type: 'literal', value: 1 } }
    const r = ev.evaluate(node, ctx({})).value as Date
    expect(r.toISOString().slice(0, 10)).toBe('2024-02-29') // 2024 闰年
  })
  it('date_add months 逆推（负 amount）', () => {
    const node: ExprNode = { type: 'date_add', unit: 'days', base: { type: 'literal', value: '2026-01-15' }, amount: { type: 'literal', value: -30 } }
    const r = ev.evaluate(node, ctx({})).value as Date
    expect(r.toISOString().slice(0, 10)).toBe('2025-12-16')
  })
  it('date_part year/month/day', () => {
    const y: ExprNode = { type: 'date_part', unit: 'year', arg: { type: 'literal', value: '2026-08-15' } }
    const m: ExprNode = { type: 'date_part', unit: 'month', arg: { type: 'literal', value: '2026-08-15' } }
    const d: ExprNode = { type: 'date_part', unit: 'day', arg: { type: 'literal', value: '2026-08-15' } }
  ...
  it('date_part day_of_week（1=周一）', () => {
    // 2026-08-15 是周六
    const node: ExprNode = { type: 'date_part', unit: 'day_of_week', arg: { type: 'literal', value: '2026-08-15' } }
  ...
  it('date_add 非法日期 → invalid_date 警告', () => {
    const node: ExprNode = { type: 'date_add', unit: 'days', base: { type: 'literal', value: 'not-a-date' }, amount: { type: 'literal', value: 1 } }
    const r = ev.evaluate(node, ctx({}))
  ...
  it('aggregate count（保持整数 number）', () => {
    const node: ExprNode = { type: 'aggregate', fn: 'count', over: { type: 'field', field: 'nums' } }
...
describe('ExprTreeEvaluator — match ReDoS 防护', () => {
  it('安全正则正常匹配', () => {
    const node: ExprNode = { type: 'string', op: 'match', left: { type: 'field', field: 'cmd' }, right: { type: 'literal', value: '^(rm|sudo)$' } }
    expect(ev.evaluate(node, ctx({ cmd: 'rm' })).value).toBe(true)
  })
  it('嵌套量词正则被 reject → false + 警告', () => {
    const node: ExprNode = { type: 'string', op: 'match', left: { type: 'field', field: 'cmd' }, right: { type: 'literal', value: '(a+)+$' } }
...
describe('ExprTreeEvaluator — 资源上限（E4）', () => {
  it('节点数超限 → 抛 ExprLimitError', () => {
    ...
    const args: ExprNode[] = Array.from({ length: 65 }, () => ({ type: 'literal', value: true } as ExprNode))
    const node: ExprNode = { type: 'and', args }
