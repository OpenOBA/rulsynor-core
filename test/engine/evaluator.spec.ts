...
describe('ERDL Evaluator  — Core decisions', () => {
  ...
  it('REQUEST_HUMAN when rule action is REQUEST_HUMAN', () => {
    const rules = [makeRule({ conditions: [{ kind: 'context_matches', field: 'tool.name', operator: 'eq', value: 'write_file' }], conditionLogic: 'OR', action: { decision: 'REQUEST_HUMAN', reason: 'Approval needed' } })]
...
describe('ERDL Evaluator  — Execution Rings', () => {
  it('Ring 0 DENY short-circuits all higher rings', () => {
    const rules = [
      makeRule({ id: 'R0', action: { decision: 'DENY', ring: 0, reason: 'Ring 0 block' }, priority: 1 }),
  ...
  it('Ring 0 EMERGENCY_HALT short-circuits immediately', () => {
    const rules = [
      makeRule({ id: 'R0', action: { decision: 'EMERGENCY_HALT', ring: 0, reason: 'halt' }, priority: 1 }),
...
describe('ERDL Evaluator  — Condition operators', () => {
  it('eq matches exact value', () => {
    const r = makeRule({ conditions: [{ kind: 'context_matches', field: 'tool.name', operator: 'eq', value: 'exec' }] })
  ...
  it('ne matches non-equal', () => {
    const r = makeRule({ conditions: [{ kind: 'context_matches', field: 'tool.name', operator: 'ne', value: 'exec' }] })
    expect(evaluator.evaluate([r], { 'tool.name': 'read' }).decision).toBe('DENY')
  })

  it('in matches array membership', () => {
    const r = makeRule({ conditions: [{ kind: 'context_matches', field: 'tool.name', operator: 'in', value: ['exec', 'bash'] }] })
  ...
  it('not_in rejects array membership', () => {
    const r = makeRule({ conditions: [{ kind: 'context_matches', field: 'tool.name', operator: 'not_in', value: ['exec', 'bash'] }], action: { decision: 'ALLOW' } })
    expect(evaluator.evaluate([r], { 'tool.name': 'search' }).decision).toBe('ALLOW')
    expect(evaluator.evaluate([r], { 'tool.name': 'exec' }).decision).toBe('ALLOW')
  })

  it('contains matches substring', () => {
    const r = makeRule({ conditions: [{ kind: 'context_matches', field: 'tool.args.command', operator: 'contains', value: 'rm' }] })
  ...
  it('not_contains rejects substring', () => {
    const r = makeRule({ conditions: [{ kind: 'context_matches', field: 'tool.args.command', operator: 'not_contains', value: 'safe' }], action: { decision: 'ALLOW' } })
    expect(evaluator.evaluate([r], { 'tool.args.command': 'dangerous' }).decision).toBe('ALLOW')
  })

  it('match supports regex', () => {
    const r = makeRule({ conditions: [{ kind: 'context_matches', field: 'tool.args.command', operator: 'match', value: '^git\\s+push.*--force' }] })
    expect(evaluator.evaluate([r], { 'tool.args.command': 'git push origin main --force' }).decision).toBe('DENY')
    expect(evaluator.evaluate([r], { 'tool.args.command': 'git push origin main' }).decision).toBe('ALLOW')
  })

  it('exists checks presence', () => {
    const r = makeRule({ conditions: [{ kind: 'context_matches', field: 'tool.args.path', operator: 'exists' }] })
  ...
  it('gt / lt for numbers', () => {
    const r = makeRule({ conditions: [{ kind: 'context_matches', field: 'step', operator: 'gt', value: 5 }] })
...
describe('ERDL Evaluator  — AND / OR logic', () => {
  it('AND: all conditions must match', () => {
    ...
    expect(evaluator.evaluate([r], { 'tool.name': 'exec', 'tool.args.command': 'rm -rf' }).decision).toBe('DENY')
    expect(evaluator.evaluate([r], { 'tool.name': 'exec', 'tool.args.command': 'ls' }).decision).toBe('ALLOW')
  })
...
describe('ERDL Evaluator  — deepContains / deepMatch', () => {
  it('deepContains finds substring in nested objects', () => {
    const r = makeRule({
      conditions: [{ kind: 'context_matches', field: 'tool.args', operator: 'contains', value: 'danger' }],
      conditionLogic: 'AND',
    })
    const result = evaluator.evaluate([r], { 'tool.args': { command: 'run-dangerous-thing', nested: { value: 'safe' } } })
    expect(result.decision).toBe('DENY')
  })

  it('deepMatch finds regex in nested objects', () => {
    const r = makeRule({
      conditions: [{ kind: 'context_matches', field: 'tool.args', operator: 'match', value: 'secret_' }],
      conditionLogic: 'AND',
    })
    const result = evaluator.evaluate([r], { 'tool.args': { key: 'secret_token', nested: { type: 'bearer' } } })
    expect(result.decision).toBe('DENY')
  })
})
...
describe('ERDL Evaluator  — CORRECT semantics', () => {
  it('CORRECT decision sets correction field', () => {
    const rules = [makeRule({
      action: { decision: 'CORRECT', correction: 'use --dry-run first', reason: 'Safe alternative' },
    })]
    const result = evaluator.evaluate(rules, { 'tool.name': 'exec' })
...
describe('ERDL Evaluator  — Instruction accumulation', () => {
  it('Multiple ALLOW rules accumulate instructions', () => {
    const rules = [
      makeRule({ id: 'R1', action: { decision: 'ALLOW', instruction: 'Log to audit' }, priority: 1 }),
...
describe('ERDL Evaluator   SPEC v1.1  3.2.2 unless exemption', () => {
  it('unless ƥ     ⣬  DENY', () => {
    const rule = makeRule({
      ...
      unless: {
        logic: 'AND',
        conditions: [{ kind: 'context_matches', field: 'user.role', operator: 'eq', value: 'admin' }],
      },
    })
    ...
    const rule = makeRule({
      ...
      unless: {
        ...
        conditions: [
          ...
          { kind: 'context_matches', field: 'file.path', operator: 'match', value: '.*\\.config\\.ts$' },
        ],
      },
    })
    // Normal user editing .config.ts   OR matches (file.path)   exempt
    const result = evaluator.evaluate([rule], { 'tool.name': 'write_file', 'user.role': 'user', 'file.path': 'vite.config.ts' })
...
describe('v1.1  6.1  ֵ ֵ ߼ ȫʧ ܣ ', () => {
  it(' ֶ ȱʧʱ !=  Ƚ Ӧ  false 򲻴 ', () => {
    const rule = makeRule({
      conditions: [{ kind: 'context_matches' as any, field: 'user.role', operator: 'ne' as any, value: 'admin' }],
  ...
  it(' ֶ ȱʧʱ gt  Ƚ Ӧ  false', () => {
    const rule = makeRule({
      conditions: [{ kind: 'context_matches' as any, field: 'api.count', operator: 'gt' as any, value: 1000 }],
  ...
  it(' ֶδ Ϊ null ʱ eq  Ƚ Ӧ  false', () => {
    const rule = makeRule({
      conditions: [{ kind: 'context_matches' as any, field: 'user.role', operator: 'eq' as any, value: 'admin' }],
      action: { decision: 'DENY', reason: 'Not admin' },
    })
  ...
  it('!= null  ֶδ Ҳ Ϊ null ʱ  true', () => {
    const rule = makeRule({
      conditions: [{ kind: 'context_matches' as any, field: 'user.role', operator: 'ne' as any, value: null }],
  ...
  it('exists  ֶ ȱʧ   false', () => {
    const rule = makeRule({
      conditions: [{ kind: 'context_matches' as any, field: 'agent.id', operator: 'exists' as any, value: true }],
  ...
  it('exists  ֶδ    true', () => {
    ...
    const result = evaluator.evaluate([rule], { 'tool.name': 'exec', 'agent.id': 'did:erdl:sha256:abc123' })
    expect(result.decision).toBe('ALLOW')
  })
  ...
  it(' ַ  "100" > 50 Ӧ  false ϸ ƥ 䣬 ʽת ', () => {
    const rule = makeRule({
      conditions: [{ kind: 'context_matches' as any, field: 'claim.amount', operator: 'gt' as any, value: 50 }],
      action: { decision: 'REQUEST_HUMAN', reason: 'High value' },
    })
    // claim.amount is string "100", not number 100
    const result = evaluator.evaluate([rule], { 'tool.name': 'claim_decision', 'claim.amount': '100' })
    expect(result.decision).toBe('ALLOW')
  })
})
...
describe('within/rate 状态累积行为（业界限流/去重语义）', () => {
  function makeRateRule(maxCount: number): RuleDefinition {
    return makeRule({
      ...
      conditions: [{ kind: 'context_matches', field: 'tool.name', operator: 'eq', value: 'exec', rate: `${maxCount}/1m` }],
      action: { decision: 'DENY', reason: 'Rate exceeded' },
    })
  }
  ...
  it('rate 不同 value 隔离（exec 不占 write_file 额度）', () => {
    ...
    const ruleA = makeRule({
      id: 'A',
      conditions: [{ kind: 'context_matches', field: 'tool.name', operator: 'eq', value: 'exec', rate: '1/1m' }],
      action: { decision: 'DENY', reason: 'A limit' },
    })
    const ruleB = makeRule({
      id: 'B',
      conditions: [{ kind: 'context_matches', field: 'tool.name', operator: 'eq', value: 'write_file', rate: '1/1m' }],
  ...
  it('within 首次放行，窗口内第二次拦截（去重语义）', () => {
    const ev = new Evaluator()
    const rule = makeRule({
      conditions: [{ kind: 'context_matches', field: 'tool.name', operator: 'eq', value: 'exec', within: '5m' }],
  ...
  it('expr 表达式树：days_between 超时效则 DENY', () => {
    ...
    expect(ev.evaluate([rule], { '确诊日期': '2020-01-01', '今天': '2026-01-01' }).decision).toBe('DENY')
  })
  ...
  it('expr 表达式树：未超时效则不匹配', () => {
    ...
    expect(ev.evaluate([rule], { '确诊日期': '2025-06-01', '今天': '2026-01-01' }).decision).toBe('ALLOW')
  })
})
