...
import {
  ...
  type DecisionTable,
} from '../../src/engine/expr-tree/decision-table.js';
...
describe('decision-table — 编译决策表到表达式树', () => {
  ...
  it('多列条件行 → and 树（所有列都匹配）', () => {
    const multiTable: DecisionTable = {
      ...
      rows: [{ conditions: { age: 60, is_retired: true }, decision: 'DENY' }],
    };
    const rows = compileDecisionTable(multiTable);
    expect(rows[0].expr.type).toBe('and');
    expect(ev.evaluate(rows[0].expr, objectContext({ age: 60, is_retired: true })).value).toBe(
      true,
    );
    expect(ev.evaluate(rows[0].expr, objectContext({ age: 60, is_retired: false })).value).toBe(
      false,
    );
  });
  ...
  it('空列/空行 → 抛错', () => {
    expect(() =>
      compileDecisionTable({ columns: [], rows: [{ conditions: {}, decision: 'X' }] }),
    ).toThrow();
  ...
  it('某行无任何条件 → 抛错', () => {
    expect(() =>
      compileDecisionTable({ columns: ['a'], rows: [{ conditions: {}, decision: 'X' }] }),
    ).toThrow();
  });
});
