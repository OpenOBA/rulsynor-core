 ...
 */
import {
  CONDITION_OPERATORS, CONDITION_MODIFIERS, ALL_OPERATORS,
  OP_COMPARE, OP_LIST, OP_STRING, OP_BOUNDARY_NEG, OP_EXISTENCE, OP_LENGTH, OP_RANGE, OP_COUNT,
  OP_COMPILE_DIRECT, OP_COMPILE_VIA_NOT, OP_COMPILE_VIA_LENGTH_COUNT,
  DO_DECISIONS, WORKFLOW_SUBSTATES, INTERNAL_REASONING, INTERNAL_STATES, RULSYNOR_EXTENSIONS, ALL_DECISIONS,
  SEMANTIC_NODES, SEMANTIC_NODE_NAMES, EXPR_NODE_TYPES, RULE_CATEGORIES,
  RULE_NAME_PREFIXES, OCCUPATION_CATEGORIES,
  SCHEMA_COUNTS, OPERATOR_ALIASES, normalizeOperatorName, isDODecision, isDecision,
  OP_VALUE_NONE, OP_VALUE_ARRAY, OP_VALUE_TUPLE, OP_VALUE_SCALAR, operatorValueShape,
...
describe('erdl-schema — 运算符（SPEC §11.1：28 条件 + 2 修饰符 = 30）', () => {
  ...
  it('§11.4 编译归宿无悬空：13 直接 + 6 not 派生 + 9 length/count = 28', () => {
    ...
    const union = new Set<string>([...OP_COMPILE_DIRECT, ...OP_COMPILE_VIA_NOT, ...OP_COMPILE_VIA_LENGTH_COUNT])
...
describe('erdl-schema — 决策（SPEC §27.5：13 种基础决策）', () => {
  ...
  it('21 = 13 + 2 + 4 + 1 + 1，无重复', () => {
    expect(ALL_DECISIONS.length).toBe(
      DO_DECISIONS.length + WORKFLOW_SUBSTATES.length + INTERNAL_REASONING.length +
      INTERNAL_STATES.length + RULSYNOR_EXTENSIONS.length,
    )
...
describe('erdl-schema — 模板运算符域 × 校验器真实放行域（2026-08-28 review 行为断言）', () => {
  it('每个模板域内的运算符都能通过校验器；域外的比较类运算符会被拒', async () => {
    ...
    for (const op of domains.fieldCompare) {
      const r = v.validate({ ...base, templateId: 'fieldCompare', params: { field: 'amount', operator: op, value: '1' } } as never)
  ...
  it('值形态分类闭合：none+array+tuple+scalar = 28，且互不重叠', () => {
    const sum = OP_VALUE_NONE.length + OP_VALUE_ARRAY.length + OP_VALUE_TUPLE.length + OP_VALUE_SCALAR.length
