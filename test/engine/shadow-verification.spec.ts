...
describe('影子验证：旧 evaluator vs 表达式树内核（单规则等价）', () => {
  const cases: Array<{ name: string; rule: RuleDefinition; context: Record<string, unknown> }> = [
    ...
    },
    {
      name: 'gt 匹配',
      rule: makeRule({
        conditions: [{ kind: 'context_matches', field: 'amount', operator: 'gt', value: 100 }],
      }),
    ...
    },
    {
      name: 'in 匹配',
      rule: makeRule({
        conditions: [{ kind: 'context_matches', field: 'cat', operator: 'in', value: ['a', 'b'] }],
      }),
    ...
    },
    {
      name: 'contains 匹配',
      rule: makeRule({
        conditions: [{ kind: 'context_matches', field: 'cmd', operator: 'contains', value: 'rm' }],
      }),
    ...
    },
    {
      name: 'exists 字段存在',
      rule: makeRule({
        conditions: [{ kind: 'context_matches', field: 'agent.id', operator: 'exists' }],
      }),
    ...
    },
    {
      ...
      rule: makeRule({
        conditions: [
          { kind: 'context_matches', field: 'meta', operator: 'eq', value: { a: 1, b: 2 } },
        ],
      }),
    ...
    },
    {
      ...
      rule: makeRule({
        conditions: [
          { kind: 'context_matches', field: 'tool.args', operator: 'contains', value: 'passwd' },
        ],
      }),
    ...
    },
    {
      ...
      rule: makeRule({
        conditions: [{ kind: 'context_matches', field: 'cmd', operator: 'match', value: 'DROP' }],
      }),
    ...
    },
    {
      ...
      rule: makeRule({
        conditions: [{ kind: 'context_matches', field: 'ver', operator: 'gt', value: '1.0' }],
      }),
    ...
    },
    {
      name: 'lt 字符串字典序',
      rule: makeRule({
        conditions: [{ kind: 'context_matches', field: 'ver', operator: 'lt', value: '2.0' }],
      }),
...
describe('新内核正确语义（有意偏离旧 bug）', () => {
  it('ne 与 eq 互补：对象同内容时 ne 应 false（旧 evaluator 的 ne 用引用比较，不互补）', () => {
    const rule = makeRule({
      conditions: [
        { kind: 'context_matches', field: 'meta', operator: 'ne', value: { a: 1, b: 2 } },
      ],
    });
  ...
  it('starts_with 严格要求 string value（旧 evaluator 做 String 隐式转换）', () => {
    const rule = makeRule({
      conditions: [
        { kind: 'context_matches', field: 's', operator: 'starts_with', value: 123 as any },
      ],
    });
