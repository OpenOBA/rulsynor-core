...
describe('RuleValidator', () => {
  ...
  describe('error level field', () => {
    it('错误级别错误应包括 level=error', () => {
      const result = validator.validate(makeInput({ templateId: 'toolEq', decision: 'DENY', params: { toolName: 'exec', when: 'true' } }))
...
describe('checkNamingConventionFull — ADR-003 前缀注册制门禁', () => {
  ...
  it('前缀已登记但格式非法（编号位数/描述大写）→ error', () => {
    expect(validator.checkNamingConventionFull(makeRule('SEC-1-Foo'))?.code).toBe('NON_STANDARD_NAME_FULL')
  })

  it('前缀表每个键都能通过格式校验（登记表自身自洽）', () => {
    for (const p of ['SEC', 'COD', 'ENG', 'PRF', 'TST', 'WRT', 'OBS', 'CUS', 'ETH', 'CMP', 'POL', 'OCC', 'CNV', 'SBP']) {
