...
function makeRule(decision: string, toolName: string, correction?: string): RuleDefinition {
  return {
    ...
    conditionLogic: 'AND',
    action: { decision: decision as RuleDefinition['action']['decision'], reason: `${decision} triggered`, correction },
    priority: 1,
    enabled: true,
  }
}
...
function makeLLM(toolName: string, rounds = 1): (msgs: unknown[]) => Promise<LLMResponse> {
  ...
  return async () => {
    call++
    if (call <= rounds) {
      return { content: 'calling tool', toolCalls: [{ name: toolName, arguments: { path: '/tmp/x' } }] }
    }
    return { content: 'done' }
  }
}
...
describe('runReActLoop — 决策穷尽分发（R3a）', () => {
  ...
  it('CORRECT 带纠偏 → 执行且参数携带纠偏内容', async () => {
    ...
    const result = await runReActLoop(opts)
    expect(executed).toHaveLength(1)
    expect((executed[0][0] as Record<string, unknown>)['__correction']).toBe('use /tmp/safe instead')
    expect(result.decision).toBe('ALLOW')
  })
...
describe('runReActLoop — previous_hash 审计链（R3b）', () => {
  it('多步执行产生逐步累积的审计哈希（每步落链）', async () => {
    ...
    const result = await runReActLoop({
      llm: async () => {
        llmCall++
        if (llmCall <= 2) return { content: 'step', toolCalls: [{ name: 'safe_tool', arguments: {} }] }
        return { content: 'done' }
      },
      evaluator: new Evaluator(),
      compiledRules,
  ...
  it('buildDecisionObject 锚定 previousAuditHash（链式语义单元验证）', () => {
    const first = buildDecisionObject({
      input: { runId: 'r', step: 0, toolName: 't', toolArgs: {}, context: {}, agentId: 'a', sessionId: 's' },
      decision: 'ALLOW', actionTaken: 'allowed', reason: null,
      matchedRules: [], totalEvaluated: 0, totalMatched: 0, rules: [], evaluationDurationMs: 1,
    })
    expect(first.audit.previous_hash).toBeNull()

    const second = buildDecisionObject({
      input: { runId: 'r', step: 1, toolName: 't', toolArgs: {}, context: {}, agentId: 'a', sessionId: 's', previousAuditHash: first.audit.hash },
      decision: 'ALLOW', actionTaken: 'allowed', reason: null,
      matchedRules: [], totalEvaluated: 0, totalMatched: 0, rules: [], evaluationDurationMs: 1,
    })
