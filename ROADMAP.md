...
**已达成**（求值错误语义 + 表达式内核对齐 spec）：

- [x] 求值错误标记 `errored: true`（E3）：除零/非法日期/元数/算术类型不匹配
- [x] `length` 标量折叠 `false`（同 aggregate 非数组）；`length(missing)` 保持 0
- [x] `in` 成员比较 NFC 归一（E10）
- [x] 664 测试

## 1.2.0（已发布 · superseded by 1.2.1）
...
```
1.0.0 ──▶ 1.1.0 ──▶ 1.2.0 ──▶ 1.2.1 ──▶ 1.3.x ──▶ 2.0.0
(试水)   (已发布) (已发布) (当前)   (功能)   (完整稳定)
```

> 说明：`1.0.0`（2026-08-07）是引擎未对齐 Spec v2.0 时的过早发布，已废弃；`1.1.0`（2026-09-07）延续版本线，取代 `1.0.0`；`1.2.0`（2026-09-08）修复 DO 审计记录缺陷（`policies[].hash` 原像、`author_id` 硬编码）+ ReDoS 检测合并 + 消硬编码；`1.2.1`（2026-09-09）修复求值错误 `errored` 标记 + `length` 标量折叠 + `in` NFC 语义。
