# OpenOBA Trademark Policy

> Full policy (Chinese master + English reference): [TRADEMARK.md](./TRADEMARK.md)
>
> Version: 1.0 | Effective: 2026-06-10
> Trademark Holder: Shenzhen Miaojing Technology Co., Ltd.

---

## Protected Trademarks

| Trademark | Status |
|-----------|--------|
| **OpenOBA™** | Pending (CNIPA Class 9, 42) |
| **ERDL™** | Pending (CNIPA Class 9) |
| **ERA-Chat™** | Pending (CNIPA Class 9, 42) |
| **Rulsynor™** | Pending (CNIPA Class 9) |
| **rulsynor-core™** | Pending (CNIPA Class 9) |
| **OpenOBA Logo** | Pending (CNIPA Class 9) |

> Status: Application pending with China National Intellectual Property Administration. ™ denotes unregistered trademark claim.

---

## Permitted Use (No License Required)

| Use | Condition |
|-----|-----------|
| Technical articles, tutorials, books | First use must note: "OpenOBA™ is a trademark of Shenzhen Miaojing Technology Co., Ltd." |
| Community events, meetups | Non-commercial only |
| Plugin compatibility declaration | "Compatible with OpenOBA™", "Built for OpenOBA™" — NOT as primary product name |
| GitHub repos containing "openoba" | Must be an actual OpenOBA-based project; README must include prominent "Unofficial" notice |
| Academic papers, media coverage | Use ™ symbol; factual descriptions only |

---

## Prohibited Use

- Impersonating official OpenOBA or Rulsynor accounts or identity
- Using OpenOBA/ERDL/ERA-Chat/Rulsynor/rulsynor-core as primary product or company name
- Domain squatting (openoba.cloud, rulsynor.io, etc.)
- Modifying the OpenOBA logo
- Implying official endorsement without written authorization

---

## Reporting Infringement

Email **postmaster@openoba.com** with:
- Infringer identity
- Evidence (screenshots, links, domain WHOIS)
- Nature of infringement

Response within 5 business days.

---

## Governing Law

People's Republic of China (Trademark Law, Anti-Unfair Competition Law).

For the complete policy including licensing applications, domain disputes, and protection measures, see the [full Trademark Policy](./TRADEMARK.md).
