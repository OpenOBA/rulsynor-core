...
npm install
npm run dev                       # 同时起 runtime + 前端 + 30 预设规则
...
**核心：源码可见，能力分层，信任换收入。** 30 预设规则匿名可拉（「不割韭菜」的技术落点），高价值规则（蓝图/合规）靠账户 + 订阅门禁。
...
3. **S3**：版本号规划（试水 1.0.0 → 正式版，建议 `2.0.0` 避免与已发布 1.0.0 冲突）
...
- `@openoba/rulsynor-core` v1.0.0（MIT，2026-08-07）是**试水版**。
- 之后 3 周 53 commit、+29,274 / −5,407 行（确定性内核重构、223 向量、合规风险条件层）。
- 版本号已自行降回 `0.1.0-alpha`（commit 3158a96），承认「1.0.0 是试水」。
- 试水版 MIT 泄露的「确定性内核 + 合规法条映射」：前者该开源，后者是公开法规的映射——**泄露有限，不影响正式版决策**。
